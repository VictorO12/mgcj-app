...
- **`scheduled-ride-digest`** *(added 2026-08-15; cron `0 21 * * *` written in `20260746` but **NOT YET REGISTERED** — verify with `SELECT jobname FROM cron.job` before assuming it runs)* — Once-daily push telling each driver how many scheduled rides are still unclaimed for **tomorrow** (a full `America/Halifax` calendar day), landing them on the Available board's "open" tab. This is the push half of the soft-claim board; the in-app half was already live via realtime. Deliberately **not** a per-ride broadcast on booking — a push per scheduled ride to every driver competes with the ride-offer channel immediate dispatch depends on, and training drivers to swipe away taxi notifications loses the 30-second offers. **Stateless**: it reports what is *open now*, not what is *new since last run* (a watermark would need another migration), so a ride open two days running is counted both days — correct, it still is. Tomorrow-only keeps it disjoint from `scheduled-release`'s 75-min horizon, so the two can never double-notify and there's no gap between them. The audience is the **one driver-read site that must not use `isDriverDispatchable()`** — see the note in the Key Technical Learnings.
