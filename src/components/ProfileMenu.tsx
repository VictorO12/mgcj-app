...
import {
  ...
  Animated, Platform, TouchableWithoutFeedback, Modal,
} from 'react-native'
...
export default function ProfileMenu({ profile, visible, onClose, onSignOut, onOpenHistory }: Props) {
  ...
  useEffect(() => {
    if (visible) {
      Animated.parallel([
        Animated.spring(slideY, { toValue: 0, useNativeDriver: true, tension: 65, friction: 12 }),
  ...
  const menuItems = [
    { icon: 'person-outline', label: 'Edit profile', sublabel: 'Name, phone number', onPress: onClose },
    { icon: 'time-outline', label: 'Ride history', sublabel: 'Past trips and receipts', onPress: () => { onClose(); onOpenHistory() } },
    { icon: 'notifications-outline', label: 'Notifications', sublabel: 'Ride updates, offers', onPress: onClose },
    { icon: 'card-outline', label: 'Payment methods', sublabel: 'Cash, card', onPress: onClose },
    { icon: 'help-circle-outline', label: 'Help & support', sublabel: 'FAQ, contact us', onPress: onClose },
  ]
    ...
    <Modal visible={visible} transparent animationType="none" onRequestClose={onClose} statusBarTranslucent>
      <View style={styles.root}>
        ...
        <Animated.View style={[styles.sheet, { transform: [{ translateY: slideY }] }]}>
          ...
          <TouchableOpacity style={styles.signOutBtn} onPress={() => { onClose(); onSignOut() }} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
  ...
  editBtn: {
    width: 34, height: 34, borderRadius: 17, backgroundColor: '#1E2A3A',
  ...
  menuIconWrap: {
    width: 36, height: 36, borderRadius: 10, backgroundColor: '#1E2A3A',
