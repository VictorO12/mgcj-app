...
export default function RideHistoryScreen({ onClose }: Props) {
  ...
  async function fetchRides() {
    ...
    if (!rideRows) { setLoading(false); return }
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statCard}>
            <Text style={[styles.statValue, cancelledCount > 0 && { color: '#E24B4A' }]}>
              {cancelledCount}
            </Text>
              ...
              <View key={ride.id} style={styles.rideCard}>
                ...
                <View style={styles.rideCardTop}>
                  <View style={styles.dateRow}>
                    ...
                    <Text style={styles.rideTime}>{formatTime(ride.created_at)}</Text>
                  </View>
                  <View style={[
                    styles.statusBadge,
                    { backgroundColor: `${STATUS_COLORS[ride.status]}18`, borderColor: `${STATUS_COLORS[ride.status]}40` }
                  ]}>
                    <Text style={[styles.statusText, { color: STATUS_COLORS[ride.status] }]}>
                  ...
                  {ride.other_party_name && (
                    <View style={styles.otherPartyRow}>
                      ...
                      <Text style={styles.otherPartyText}>
                        {isDriver ? 'Passenger' : 'Driver'}: {ride.other_party_name}
                      </Text>
                    </View>
                  )}
...
const styles = StyleSheet.create({
  ...
  emptySubtitle: { fontSize: 13, color: '#374151', textAlign: 'center', paddingHorizontal: 40 },
  ...
  routeLine: { width: 1.5, height: 12, backgroundColor: 'rgba(255,255,255,0.1)', marginLeft: 2 },
